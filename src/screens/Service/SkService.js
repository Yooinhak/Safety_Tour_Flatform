...
function DensityEmotion({dt}) {
    ...
    if( dt <= 30) {
        return (
            <Image
                style={styles.logo}
                source={require('./image/great.png')}
            />
        )
    } else if( dt <= 60) {
        return (
        ...
        return (
            <Image
                style={styles.logo}
                source={require('./image/notGood.png')}
            />
        )
    } else {
        return (
    ...
    return  (
            <View style={styles.container}>
                <View style={styles.textContainer}>
                    <Text>{str.substr(4,2)}월 {str.substr(6,2)}일 </Text>
                    <Text>{covidInfo.hh}시 기준</Text>
                </View>
                <View style={styles.imageContainer}>
                    <DensityEmotion dt={covidInfo.contactDensityPercentile} />
                    <Text>현재 코로나 안전 수치는 {Math.ceil(covidInfo.contactDensityPercentile)}</Text>
                </View>
